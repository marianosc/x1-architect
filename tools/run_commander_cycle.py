# Ejecuta UN ciclo del commander (XAUUSD @ H1) instrumentado con tiempos por
# capa, sin bucle infinito ni input() interactivo. Llama EXACTAMENTE los mismos
# scripts y en el mismo orden que commander.run_main_commander (L1 -> 8x(L2,L3)
# -> L5,L4). Vuelca cycle_timings.json para la autopsia.
import os, sys, subprocess, time, json, glob
from pathlib import Path

# Forzar UTF-8 en este proceso y en TODOS los hijos (L1/L2/L3/L4/L5): cuando la
# salida se redirige a archivo, Windows usa cp1252 y los emojis (🚀/✅) de L2
...
DATA = BASE / "data"
PY = sys.executable
SYM, TF, TFOPT = "XAUUSD", "H1", "0"

csvs = [f for f in glob.glob(str(DATA / "*.csv")) if "XAU" in os.path.basename(f).upper()]
if not csvs:
    print("No encuentro el CSV de XAUUSD en data/"); sys.exit(1)
TARGET_CSV = csvs[0]
...
for side in ["LONG", "SHORT"]:
    for fam in ["MOMENTUM", "TREND", "VOLATILITY", "CYCLE"]:
        ...
        (BASE / "cycle_timings.json").write_text(json.dumps(T, indent=2))  # checkpoint

T["L5"] = run("L5 consolidacion", [str(BASE / "L5.py"), SYM, TF])
T["L4"] = run("L4", [str(BASE / "L4.py")])
T["total_s"] = round(time.time() - t_cycle0, 1)
(BASE / "cycle_timings.json").write_text(json.dumps(T, indent=2))
